...
Ramped version is applied if all of these are true 
    ...
    - Driving fields for the first and last simulations are on the ascending order when compared by their absolute values 
    - `Bext_step` is nonzero 
...
If any of the conditions above is false, regular simulation is run
    ...
    - `Bext_step`, `Bext_end` and `nsteps_at_ramp` values have no impact on the simulation 
    - Simulation is run with given `B_ext` for `nsteps` iterations 


In ramped version

    - Simulation is run for `nsteps_at_ramp` iterations at each driving field value of `B_ext=n*Bext_step, where n=0,1,...` until `Bext_end` is achieved 

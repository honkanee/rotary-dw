...
void HDF5Writer::make_output_file()
    {
        try {
            ...
            Real Bext = p.Bext_SI();
            H5::Attribute attribute_Bext = output_file.createAttribute("Bext_mT", H5Type<Real>::type(), attribute_dataspace);
            attribute_Bext.write(H5Type<Real>::type(), &Bext);

            Real sigma = p.sigma_SI();
            H5::Attribute attribute_sigma = output_file.createAttribute("sigma_mT", H5Type<Real>::type(), attribute_dataspace);
            attribute_sigma.write(H5Type<Real>::type(), &sigma);

            Real delta_t = p.dt_SI();
            H5::Attribute attribute_Delta_t = output_file.createAttribute("Delta_t_ns", H5Type<Real>::type(), attribute_dataspace);
            attribute_Delta_t.write(H5Type<Real>::type(), &delta_t);

            Real dmi_const = p.dmi_const_SI();
            H5::Attribute attribute_dmi_const = output_file.createAttribute("DMI_const_J_per_m2", H5Type<Real>::type(), attribute_dataspace);
            attribute_dmi_const.write(H5Type<Real>::type(), &dmi_const);
            ...
            h_dataset = output_file.createDataSet("h", H5Type<Real>::type(), *dataspace, createParams);
            ...
            phi_dataset = output_file.createDataSet("phi", H5Type<Real>::type(), *dataspace_phi, createParams);
            ...
            if (p.ramped_field) {
                ...
                Bext_dataset = output_file.createDataSet("Bext", H5Type<Real>::type(), *dataspace_Bext, createParams_Bext);
            }
            ...
            v_mean_dataset = output_file.createDataSet("v_mean", H5Type<Real>::type(), *dataspace_v_mean, createParams_v_mean);
            ...
            Real dt_v_mean_ns = p.calc_v_frec*p.dt_SI()*1e9;
            H5::Attribute attribute_dt_v = v_mean_dataset.createAttribute("dt_ns", H5Type<Real>::type(), attribute_dataspace);
            attribute_dt_v.write(H5Type<Real>::type(), &dt_v_mean_ns);
...
void HDF5Writer::write(int step_count)
    {
        ...
        std::vector<Real> h_in_um(h_host.extent(0));
        std::vector<Real> phi(h_host.extent(0));
        for (size_t i = 0; i < h_host.extent(0); ++i) {
        ...
        h_dataset.write(h_in_um.data(), H5Type<Real>::type(), *memspace, *h_filespace);
        delete h_filespace;
        delete memspace;
        ...
        phi_dataset.write(phi.data(), H5Type<Real>::type(), *phi_memspace, *phi_filespace);
        ...
        if (p.ramped_field)
        {
            ...
            //Real Bext_mT = *B_ptr*(*m_dw).mu0*(*m_dw).Ms*1000;
            //Bext_dataset.write(&Bext_mT, H5Type<Real>::type(), *Bext_memspace, *Bext_filespace);
            //delete Bext_memspace;
            //delete Bext_filespace;
        }

        std::vector<Real> v_mean_in_m_per_s(s.v_idx);
        ...
        v_mean_dataset.write(v_mean_in_m_per_s.data(), H5Type<Real>::type(), *v_mean_memspace, *v_mean_filespace);
        delete v_mean_memspace;
        delete v_mean_filespace;

        ++rows;

        std::chrono::duration<Real> run_time = std::chrono::system_clock::now()-run_clock_start;
